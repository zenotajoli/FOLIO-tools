...
global adhocFormats
adhocFormats = set (["circulation_rules"])
...
class Purge:
    ...
    def countrecs(self, j, config):
        if config['name'] in adhocFormats:
            if config['name'] == 'circulation_rules':
                return 1
        else:
            try:
                if j['totalRecords']:
                    return int(j['totalRecords'])
                else:
                    return 0
            except Exception:
                return 0
    ...
    def purge_one_setting(self, config):
        ...
        total_recs = self.countrecs(j, config)
        if config['name'] in adhocFormats:
            if config['name'] == 'circulation_rules':
                res = j
        else:
        ...
        if config['name'] in adhocFormats:
            if config['name'] == 'circulation_rules':
                purge.purge_circulation_rules (res, config)
        else:
            for i in res:
                try:
    ...
    def purge_circulation_rules (self, res, config):
        #Now DELETE method is not present, so it end always with an error
        try:
            ident = res["id"]
            url = self.folio_client.okapi_url + config["path"] + "/" + ident
            print(url, flush=True)
    ...
    def restore_one_setting(self, config):
        ...
        if path.exists():
            ...
            with pathlib.Path.open(path) as refdata_file:
                ...
                if config['name'] in adhocFormats:
                    if config['name'] == 'circulation_rules':
                        restore.restore_circulation_rules(config, refdata["data"])
                else:
            ...
            print("File doesn't exist. Go to next.")

    def restore_circulation_rules(self, config, datajson):
        try:
            url = self.folio_client.okapi_url + config["path"]
            headers = self.folio_client.okapi_headers
            if config["insertMethod"] == "put":
                req = requests.put(url, data=json.dumps(datajson), headers=headers)
                print(req.status_code)
            if config["insertMethod"] == "post":
                print("ERROR=================================")
                print("POST method do not work for circulation_rules")
        except Exception as ee:
                print("ERROR=================================")
                print(ee)   
